...
async def gben(userbot):
    ...
    if not sender.id == me.id:
        dark = await dc.reply("`Memulai Proses Gban Si dia!`")
    else:
        dark = await dc.edit("`Memproses Global Banned Si dia ini!!!`")
    ...
    if user:
        if user.id == 1547381425:
            return await dark.edit(
                f"`ANDA GABISA GBAN TUAN MUDA KIM, DIA ADALAH SULTANNN!!!`"
            )
        try:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await dark.edit(f"`Global Banned Aktif ya tuan✅`")
            except BaseException:
                b += 1
    else:
        await dark.edit(f"`Reply pesan dulu cooooook!!`")
    try:
...
async def gunben(userbot):
    ...
    if user:
        if user.id == 1547381425:
            return await dark.edit("**LU GABISA BLACKLIST kim, DIA MAJIKANN GUA !!!**")
        try:
